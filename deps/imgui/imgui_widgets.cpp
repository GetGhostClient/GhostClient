...
// [Ghost Client patch] Syntax colorize hook definition
ImGui::ImGuiSyntaxColorizeFunc ImGui::GhostSyntaxColorize = nullptr;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            // [Ghost Client patch] use syntax colorize hook when set
            if (is_multiline && !is_displaying_hint && ImGui::GhostSyntaxColorize)
                ImGui::GhostSyntaxColorize(draw_window->DrawList, g.Font, g.FontSize,
                    draw_pos - draw_scroll, buf_display, buf_display_end);
            else
